...
--// PATCH: Based on runnable PRESERVED_FIX structure, Rushex connected to Menu 4 aim state.
...
local function getMenu4Target()
    if not RivalsUI.Aimbot and not RushexEnabled then
        return nil
    end
...
local function UpdateMenu4Aimbot()
    -- Rushex tersambung ke Menu 4 aim.
    -- Kalau Rushex ON, aim loop boleh jalan dari tombol GUI/mobile tanpa wajib klik kanan.
    -- Kalau Rushex OFF, behavior lama tetap: AIMBOT butuh MouseButton2.
    if not RivalsUI.Aimbot and not RushexEnabled then
        return
    end
...
local function UpdateMenu4UI()
    ...
    if Menu4StatusLabel then
        Menu4StatusLabel.Text = string.format(
            "Aimbot: %s  |  ESP: %s  |  Fly: %s  |  Rushex: %s  |  RUSHEX: %s",
...
local function SetRushexEnabled(value)
    ...
    -- Sambungkan Rushex ke AIMBOT/Menu 4 tanpa mengubah struktur GUI lama.
    RivalsUI.Aimbot = RushexEnabled
    OnMenu4AimbotChanged(RushexEnabled)

    UpdateRushexFloatingButton()
    UpdateMenu4UI()

    Notify("RUSHEX", RushexEnabled and "ON - Aim Menu 4 aktif" or "OFF - Aim Menu 4 mati", 2)
end
...
Menu4StatusLabel = AddMenu4ThemeLabel(New("TextLabel", {
    Size = UDim2.new(1, 0, 0, 36),
    BackgroundTransparency = 1,
    Text = "Aimbot: OFF  |  ESP: OFF  |  Fly: OFF  |  Rushex: OFF  |  RUSHEX: OFF",
    TextColor3 = GetTheme().Text,
    Font = Enum.Font.GothamBold,
    TextSize = 12,
    TextWrapped = true,

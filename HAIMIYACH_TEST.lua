...
--// FIX: RUSHEX CONNECTED TO MENU 4 AIM
...
--// FORCE SHOW GUI FIX
--// Wajib jalan di LOCAL context. Kalau LocalPlayer belum siap, tunggu dulu.
...
local function AddGuiParent(parent)
    ...
    for _, savedParent in ipairs(GuiParents) do
        if savedParent == parent then
            return
        end
    end
    table.insert(GuiParents, parent)
end

-- PlayerGui dibuat prioritas agar langsung terlihat di LocalScript normal.
-- gethui/CoreGui tetap disiapkan sebagai fallback executor.
...
AddGuiParent(CoreGui)

local GuiParent = PlayerGui or GuiParents[1]
...
local LAUNCHER_GUI_NAME = "Haimiyach_Premium_Launcher"
for _, parent in ipairs(GuiParents) do
    pcall(function()
        local oldGui = parent:FindFirstChild(OLD_GUI_NAME)
        if oldGui then
            oldGui:Destroy()
        end
    end)
    ...
    pcall(function()
        local oldLauncher = parent:FindFirstChild(LAUNCHER_GUI_NAME)
        if oldLauncher then
            oldLauncher:Destroy()
        end
    end)
...
local Menu4RushexToggleButton = nil
...
local function New(className, props)
    ...
    if not ok or not obj then
        warn("[HAIMIYACH GUI] gagal membuat instance:", tostring(className), tostring(obj))
        return nil
    end

    local parent = props and props.Parent or nil
    ...
    for key, value in pairs(props or {}) do
        if key ~= "Parent" then
            ...
            if not propOk then
                warn("[HAIMIYACH GUI] skip property", tostring(className) .. "." .. tostring(key), tostring(propErr))
            end
        end
    end
    ...
    if parent then
        ...
        if not parentOk then
            warn("[HAIMIYACH GUI] gagal set parent", tostring(className), tostring(parentErr))
        end
    end
...
local function BindAutomaticCanvas(scrollFrame, listLayout, extraBottom)
    if not scrollFrame or not listLayout then
        return
    end

    local function updateCanvas()
        pcall(function()
            scrollFrame.CanvasSize = UDim2.new(0, 0, 0, listLayout.AbsoluteContentSize.Y + (extraBottom or 16))
        end)
    end
...
local btn = New("TextButton", {
    ...
    btn.MouseButton1Click:Connect(function()
        if callback then
            ...
            if not ok then
                warn("[HAIMIYACH GUI] button error:", tostring(text), tostring(err))
                Notify("Error", "Tombol " .. tostring(text) .. " error. Cek console.", 3)
            end
        end
    end)
...
--// FORCE OPEN LAUNCHER
--// Jika GUI utama tidak terlihat karena parent diblokir executor/game, tombol kecil ini dibuat di semua parent yang mungkin.
local LauncherButtons = {}
local function ForceShowMainGui(parent)
    if ScreenGui then
        pcall(function()
            ScreenGui.Enabled = true
        end)

        if parent and ScreenGui.Parent ~= parent then
            pcall(function()
...
local function CreateForceLauncher()
    for _, parent in ipairs(GuiParents) do
        local launcherGui = New("ScreenGui", {
            Name = LAUNCHER_GUI_NAME,
            ResetOnSpawn = false,
            IgnoreGuiInset = true,
            Enabled = true,
            DisplayOrder = 10000,
        ...
        if launcherButton then
            ...
            Stroke(launcherButton, Color3.fromRGB(255, 255, 255), 1, 0.35)
            table.insert(LauncherButtons, launcherButton)

            launcherButton.MouseButton1Click:Connect(function()
                ForceShowMainGui(parent)
            end)
        end
    end
end
...
BindAutomaticCanvas(PlayerScrollFrame, PlayerListLayout, 20)
...
BindAutomaticCanvas(SettingsScroll, SettingsList, 24)
...
BindAutomaticCanvas(EmoteScroll, EmoteListLayout, 20)
...
BindAutomaticCanvas(Menu3Scroll, Menu3List, 20)
...
local function getMenu4Target()
    -- RUSHEX CONNECTED: target aktif jika Aimbot ON atau Rushex ON.
    if not (RivalsUI.Aimbot or RushexEnabled) then
        return nil
    end
...
local function UpdateMenu4Aimbot()
    -- RUSHEX CONNECTED:
    -- Aimbot manual tetap butuh MouseButton2.
    -- Rushex ON langsung mengaktifkan aim dari tombol GUI/mobile.
    if not (RivalsUI.Aimbot or RushexEnabled) then
        return
    end

    if not RushexEnabled and not UserInputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton2) then
        return
    end
...
--// RUSHEX MENU 4 CONTROLLER
--// Dimasukkan sebagai controller Menu 4 yang rapi:
--// 1x GUI, 1x toggle state, tidak membuat koneksi berulang.
local function UpdateRushexFloatingButton()
    ...
    if Menu4RushexToggleButton then
        Menu4RushexToggleButton.Text = RushexEnabled and "RUSHEX: ON" or "RUSHEX: OFF"
    end
...
local function SetRushexEnabled(value)
    RushexEnabled = value == true

    -- RUSHEX CONNECTED KE AIM:
    -- Saat Rushex ON, Aimbot Menu 4 ikut ON.
    -- Saat Rushex OFF, Aimbot Menu 4 ikut OFF supaya status tidak bentrok.
    RivalsUI.Aimbot = RushexEnabled
    StartMenu4Loop()

    -- State global supaya script lain bisa baca status fitur ini.
    HaimiyachEnv.RushexEnabled = RushexEnabled
    HaimiyachEnv.NgeweenakEnabled = RushexEnabled
    HaimiyachEnv.Ahahenak = RushexEnabled
    HaimiyachEnv.CamlockState = RushexEnabled

    UpdateRushexFloatingButton()
    UpdateMenu4UI()

    Notify("RUSHEX", RushexEnabled and "ON + AIM CONNECTED" or "OFF + AIM OFF", 2)
end
...
local function CreateRushexFloatingButton()
    ...
    RushexFloatingButton.MouseButton1Click:Connect(function()
        SetRushexEnabled(not RushexEnabled)
    end)
...
local function ResetRushexMenu4()
    SetRushexEnabled(false)

    if RushexFloatingButton then
        RushexFloatingButton:Destroy()
        RushexFloatingButton = nil
    end
...
BindAutomaticCanvas(Menu4Scroll, Menu4List, 24)
...
Menu4StatusLabel = AddMenu4ThemeLabel(New("TextLabel", {
    Size = UDim2.new(1, 0, 0, 36),
    BackgroundTransparency = 1,
    Text = "Aimbot: OFF  |  ESP: OFF  |  Fly: OFF  |  RUSHEX: OFF",
    TextColor3 = GetTheme().Text,
    Font = Enum.Font.GothamBold,
    TextSize = 12,
    TextWrapped = true,
...
Menu4RushexToggleButton = MakeButton("RUSHEX: OFF", Menu4Scroll, function()
    SetRushexEnabled(not RushexEnabled)
end)
Menu4RushexToggleButton.Size = UDim2.new(1, 0, 0, 34)
...
local Menu4ResetButton = MakeButton("RESET SETTINGS", Menu4Scroll, function()
    ...
    RushexEnabled = false
    HaimiyachEnv.RushexEnabled = false
    HaimiyachEnv.NgeweenakEnabled = false
    HaimiyachEnv.Ahahenak = false
    HaimiyachEnv.CamlockState = false
    UpdateRushexFloatingButton()
...
--// FORCE VISIBLE INIT
ForceShowMainGui(GuiParent)
CreateForceLauncher()

--// AUTO SHOW RUSHEX FLOATING BUTTON SAAT SCRIPT DI-RUN
Defer(function()
    if CreateRushexFloatingButton then
        CreateRushexFloatingButton()
        if RushexFloatingButton then
            RushexFloatingButton.Visible = true
        end
...
-- ulangi beberapa kali agar tetap muncul kalau parent GUI lambat siap
Defer(function()
    for _ = 1, 6 do
        if task and task.wait then
            task.wait(0.5)
